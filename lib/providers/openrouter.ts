...
${request.fileContent}`,
        ...
        if (!response.ok) {
            ...
            throw new Error(error.error?.message || "Failed to review document with OpenRouter")
        }
        ...
        if (!fullReview) {
            throw new Error("No text content found in OpenRouter API response")
        }
    ...
    async *reviewStream(_request: ReviewRequest): AsyncGenerator<string> {
        yield "[Streaming not yet implemented for OpenRouter provider — Step 4]"
    }
