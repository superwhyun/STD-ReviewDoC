# Step 5: runtime-smoke-and-phase-close
...
- owned_paths:
  ...
  - `projects/DraftReviewr/phases/1-mvp-hardening/step5-output.json`
- read_contracts:
  - `projects/DraftReviewr/package.json`
  - `projects/DraftReviewr/phases/1-mvp-hardening/step1-output.json`
  - `projects/DraftReviewr/phases/1-mvp-hardening/step2-output.json`
...
- 이 step은 phase 마감 step이다.
- 구현 파일은 수정하지 않는다.
- `/`, `/admin`, `/settings`가 빌드 후 런타임에서 접근 가능해야 한다.
- phase 완료 시 `phases/baselines/1-mvp-hardening.json`을 작성한다.
- phase 완료 후 `draft-reviewer-phase1-done` tag를 생성한다.

## 작업

1. Step 1-4 output을 종합해 runtime smoke 검증 범위를 확정한다.
2. 개발 서버 또는 빌드 결과를 기준으로 `/`, `/admin`, `/settings`가 열리는지 확인한다.
3. 첫 실행 기본 데이터가 문서 검토/관리자 화면에서 접근 가능한지 확인한다.
4. 수정된 검증 script와 build/typecheck를 실행한다.
5. `phases/baselines/1-mvp-hardening.json`에 변경된 public surface, integration points, known issues를 기록한다.
6. `phases/index.json`, `phases/project-manifest.json`, `phases/1-mvp-hardening/index.json` 상태를 완료로 동기화한다.
...
- [ ] `/`, `/admin`, `/settings` smoke 확인 결과를 `step5-output.json`에 기록한다.
- [ ] `phases/baselines/1-mvp-hardening.json`이 변경된 public surface, routes, integration points, known issues를 요약한다.
- [ ] `phases/index.json`과 `phases/1-mvp-hardening/index.json` 상태가 일치한다.
- [ ] phase 완료 후 `git tag draft-reviewer-phase1-done`을 생성한다.
...
5. `phases/1-mvp-hardening/index.json` 과 `step5-output.json` 에 상태와 handoff를 기록한다.
6. step 커밋 후 phase 완료 태그를 생성한다.

## 금지사항

- 구현 파일을 이 step에서 수정하지 마라.
- 실패한 smoke 결과를 숨기지 마라. 실패하면 step을 `error` 또는 `blocked`로 기록하고 다음 액션을 남겨라.
